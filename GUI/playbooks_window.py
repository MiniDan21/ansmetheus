...
class RunLogDialog(QDialog):
	def __init__(self, host, file):
		...
		btn = QPushButton("Закрыть"); btn.setFont(QFont("Arial", 13))
...
class PlaybookRunner(QThread):
	...
	def __init__(self, host, inv, pb):
		super().__init__()
		self.host, self.inv, self.pb = host, inv, pb

	def run(self):
		try:
			cmd = [sys.executable, PLAYBOOK_CLI, self.host, "-i", self.inv, "-p", self.pb]
			p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

			for l in p.stdout:
				self.log.emit(l.rstrip())
			for l in p.stderr:
				self.log.emit("❗ " + l.rstrip())
...
class TaskEditor(QDialog):
	...
	def render(self):
		...
		for arg, cfg in spec.items():
			f = QLineEdit(); f.setFont(QFont("Arial", 13))
			if "default" in cfg:
...
class PlaybooksWindow(QMainWindow):
	...
	def edit_pb(self):
		...
		names = [self.tree.topLevelItem(i).text(0)
	...
	def delete_item(self):
		...
		if "task" in info:
			pb = yaml.safe_load(open(file, encoding="utf-8"))
			pb[0]["tasks"] = [
				t for t in pb[0]["tasks"] if t["name"] != info["task"]["name"]
			]
			self.dump(file, pb)
		else:
			os.remove(file)
